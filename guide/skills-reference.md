...
| `/harvest-findings {N}` | Composable manual-test-pass loop closer. After a tester records findings as inline bullets under each scenario in `docs/epics/epic-{N}-test-plan.md`, harvests them into the `docs/epics.md` backlog (durable, idempotent per test-pass date), triages each as **[in-scope]** vs **[defer]**, authors a single remediation story `{N}.{last+1}` (one AC per in-scope finding, each linked to its source scenario) via `/create-story` + GitHub tracking, routes deferred findings through `/deferred`, then resets the plan for re-test. **Never** reopens a `done` story — fixes always land in the new story. Called at the boundary by `/epic-flywheel` and `/retrospective`; directly invocable. |
