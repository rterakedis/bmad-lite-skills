...
Directly invocable as `/docs-sync` (runs OPERATIONAL over the working tree). Three doc audiences kept deliberately separate: human operational guides (OPERATIONAL writes), LLM planning doc (PROMOTE writes), external-sourced coding guidance (neither writes — DRIFT only flags; `/refresh-*` owns it). Surfaced through the `DOCS UPDATED:` developer-report field, the `docs_updated` ledger field, and a dev-story DoD item.
