...
Run **OPERATIONAL** over the current working tree — derive the changed-file list with `git diff --name-only` against the last commit (or the merge-base of the current branch), then apply the op and report what changed. Use this to catch the human guides up after manual work done outside the flywheel.
