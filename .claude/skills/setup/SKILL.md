---
name: setup
description: Scaffold the docs folder structure, create AGENTS.md and CLAUDE.md. Also handles migrating existing full-BMAD projects to BMAD-LITE. Use when starting a new project or when the user says "setup project", "init docs", "migrate", or "setup migrate/clean".
---
...
| `_bmad/` exists | Report: "Found full BMAD infrastructure. Run `/setup migrate` to move artifacts to BMAD-LITE layout, then `/setup clean` to remove BMAD files." Stop. |
...
Move full-BMAD artifacts into BMAD-LITE layout. **Non-destructive** — only moves/copies, never deletes. Run `/setup clean` afterward to remove BMAD files.
...
For each file, move to BMAD-LITE canonical path if not already there. Skip if destination exists (never overwrite).

| Source (BMAD) | Destination (BMAD-LITE) |
|---|---|
...
BMAD stores story status centrally in `sprint-status.yaml`, not in individual story files. BMAD-LITE story files carry their own `Status:` frontmatter field, which is what `/github-tracking backfill` reads to set GitHub labels. Without this step, every migrated story would appear as `ready-for-dev` regardless of its actual state.
...
For each story file moved to `docs/epics/` in Step 4, derive the sprint-status.yaml key by stripping the `.md` extension from the filename (e.g., `1-1-user-authentication.md` → key `1-1-user-authentication`). Look up that key under `development_status` in `sprint-status.yaml`. Map the BMAD status to the BMAD-LITE `Status:` value:
...
Run **Scaffold Flow** Step 2 + Step 3 to add any BMAD-LITE sections missing from existing files. Never replace existing content.
...
> "Delete `src/bmm-skills/` (the original BMAD skill source files)? BMAD-LITE skills in `skills/` are the replacement. (y/n)"
